	...
	<div class="card shadow mb-4">
		<div class="card-header d-flex justify-content-between align-items-center font-weight-bold text-primary">
			...
			<div class="ml-auto text-end" style="font-size:12px;">
				<div class="row">
					<a id="Add" href="<?php echo site_url('Lokasi/create');?>" class="btn btn-sm btn-primary">
				...
				<table id="datatables" class="display table table-striped table-hover">
					...
					if($mdata->getNumRows() > 0){
						...
						foreach ($mdata->getResult() as $rows) {
							...
							<td>'.$rows->lokasi.'</td>
							<td>'.$rows->alamat.'</td>
							<td>'.$rows->latitude.','.$rows->longitude.'</td>
							<td>
								<div class="form-button-action">
									<div class="btn-group-justified">
										<a id="Edit" type="button" class="btn btn-warning btn-sm btn-circle" href="' . site_url('Lokasi/edit/' . trim(base64_encode($rows->id_lokasi), '=') . '') . '"  data-toggle="modal" data-target="#form-modal">
											<i class="ace-icon fa fa-edit bigger-130"></i>
										</a>
										<a id="Remove" type="button" class="btn btn-danger btn-sm btn-circle" data-toggle="modal" data-target="#confirm-delete" data-href="' . site_url('Lokasi/remove/' . trim(base64_encode($rows->id_lokasi), '=') . '') . '">
